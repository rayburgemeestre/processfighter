    ...
    while (running) {
      ...
      if (global_game_state_.recv_socket().receive(packet, ipaddr, port) == sf::Socket::Done) {
        ...
        switch (id) {
          ...
          case static_cast<sf::Int32>(messages::message_types::challenge): {
            std::unique_ptr<messages::message_interface> m = std::make_unique<messages::challenge> (global_game_state_);
            if (!m->parse(packet, ipaddr)) {
              continue;
            }
            receive(m);
